...
if (strlen($_SESSION['feedid']==0)) {
  header('location:logout.php');
    ...
    if(isset($_POST['Theory'])){
        $ch1=$_POST['Tid'];
        $asd="select user_name from users where full_name='$ch1'";
        ...
        $de1=$r3["program"];
        if($de==$de1){
            header("Location: choice_th.php?code=$ch");
        }
        else{
            $msg="You donot have access to details of $de1";
        }
    }
    else if(isset($_POST['Practical'])){
        $ch1=$_POST['Tid'];
        $asd="select user_name from users where full_name='$ch1'";
        $resq=mysqli_query($con, $asd);
        $q = mysqli_fetch_array($resq);
...
$(document).ready(function(){
    $('.search-box input[type="text"]').on("keyup input", function(){
        ...
        if(inputVal.length){
            $.get("backend_staff.php", {term: inputVal}).done(function(data){
                                            ...
                                            <form action="" method="post" enctype="multipart/form-data" class="form-horizontal">
                                                ...
                                                <div class="row form-group">
                                                    <div class="col col-md-3">
                                                         <label for="text-input" class=" form-control-label">Select Faculty</label><br>
                                                    </div>
                                                    <div class="col-12 col-md-9">
                                                        <div class="search-box">
                                                            <input type="text" id="Tid" name="Tid" class="form-control" autocomplete="off" placeholder="Search staff..."/>
                                                            <div class="result"></div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="card-footer"><table style="margin-left:300px">
                                                    <td><p style="text-align: center;"><button type="submit" name="Theory" id="Theory" class="btn btn-primary btn-sm" >Theory</button></p></td>
                                                    <td><p style="text-align: center;"><button type="submit" name="Practical" id="Practical" class="btn btn-primary btn-sm">Practical</button></p></td>
