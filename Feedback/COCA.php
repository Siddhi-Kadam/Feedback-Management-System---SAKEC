...
if (strlen($_SESSION['feedid']==0)) 
{
  header('location:logout.php');
    ...
    if($nococa==0)
    {
        ...
        $r=mysqli_query($feeddbc,"INSERT INTO coafeedback(std_id,division,nococa) VALUES('$feedid','$di','$nococa')");
        for ($j = 1; $j <13; $j++) {
            if(isset($_POST['Q' . $j])){
                $a = $_POST['Q' . $j];   
                $r=mysqli_query($feeddbc,"UPDATE coafeedback set Q$j=0 where std_id='$feedid' and division='$di' and nococa='$nococa'");
            }
        }
    }
    if(isset($_POST['Submit'])){
        if(isset($_FILES['image'])){
            ...
            if(empty($errors)==true){
                move_uploaded_file($file_tmp,"Certificate/COCA/".$file_name);
                echo "Success";
            }
            else{
                print_r($errors);
                    ...
                    <div class="card-body card-block">
                        <form method="post" enctype="multipart/form-data" class="form-horizontal">
                            ...
                            <div class="row form-group" style="margin-left:200px">
                                ...
                                <tr>
                                        <th rowspan="2"><center>Co-Curricular Activity</center></th>
                                    </tr>
                                    <tr><th>Ratings</th></tr>
                                        <?php
                                    ...
                                    if(isset($_POST['Submit'])){
                                        ...
                                        for ($j = 1; $j <13; $j++) {
                                            if(isset($_POST['Q' . $j])){
                                                $a = $_POST['Q' . $j];   
                                                $r=mysqli_query($feeddbc,"UPDATE coafeedback set Q$j='$a' where std_id='$feedid' and division='$di'");
                                            }
                                        }
                                    }?>
                            </div>
